...
void ssd1803Decode(uint8_t *data, uint32_t data_length, uint16_t instruction_code, ssd1803_instruction_t *instruction)
{
    ...
    // Set re, if required
    if (instruction_code & SSD1803_SET_RE0)
    {
        instruction->set_re = true;
        instruction->re = false;
    }
    else if (instruction_code & SSD1803_SET_RE1)
    {
        instruction->set_re = true;
        instruction->re = true;
    }
    ...
    // Set is, if required
    if (instruction_code & SSD1803_SET_IS0)
    {
        instruction->set_is = true;
        instruction->is = false;
    }
    else if (instruction_code & SSD1803_SET_IS1)
    {
        instruction->set_is = true;
        instruction->is = true;
    }
    ...
    instruction->payload[0] = SSD1803_SPI_START_BYTE_LSB_ORDER |
    ...
    if (data_length && data)
    {
        ...
        {
            instruction->payload[1 + 2 * d] = data[d] & 0xf;            // lower 4 bit
...
{
    uint8_t data = instruction_code & 0xff;

    ssd1803Decode(&data, 1, instruction_code, instruction);
}

void ssd1803_clear_display(ssd1803_instruction_t *instruction)
{
    ssd1803DecodeInstruction(SSD1803_CLEAR_DISPLAY, instruction);
}

void ssd1803_return_home(ssd1803_instruction_t *instruction)
{
    ssd1803DecodeInstruction(SSD1803_RETURN_HOME, instruction);
}
...
{
    uint16_t code = SSD1803_ROM_SELECT;
